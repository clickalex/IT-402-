...
("Which image method keeps the .odt file small?", ["Embed", "Link (references an external image file)", "Copy-paste", "Drag-drop"], "Link (references an external image file)"),
...
("Which grouping order supports reliable department-wise subtotals?", ["Filtered", "Sorted by the grouping column", "Printed", "Shared"], "Sorted by the grouping column"),
("Which location can store a user’s LibreOffice macros?", ["My Documents", "My Macros", "Templates", "Recycle Bin"], "My Macros"),
...
("On a data-bound Base form, Label versus Text Box…", ["Both store data", "Label shows fixed text, Text Box edits a linked field", "Both print only", "Label stores, Text Box shows"], "Label shows fixed text, Text Box edits a linked field"),
]),
...
("If it can be done safely, which action isolates the electrical supply before contact-based assistance?", ["Touch the victim", "Throw water", "Switch OFF power", "Call a friend"], "Switch OFF power"),
("Which is a typical starting position for a desktop screen?", ["20–30 cm away", "About 50–70 cm away, with the top at or slightly below eye level", "2 metres away", "Below the desk"], "About 50–70 cm away, with the top at or slightly below eye level"),
